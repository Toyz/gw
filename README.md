...
| `gw doc [unit] [symbol]` | Show a unit's documentation: a Go module docs via `go doc <import-path> [symbol]` (resolved from a short name like `api`, or `<unit>/<subpkg>`, with go's `-all`/`-src`/`-u`/`-c` forwarded); a project docs via its toolchain's `doc` task (e.g. rust → `cargo doc`). With no unit, lists what's documentable — the workspace root has no package of its own, so bare `go doc` there fails. |
